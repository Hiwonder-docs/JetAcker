...
Load the model from Mediapipe, input the image into it, and use OpenCV to display the output image.
...
Load the model from Mediapipe, input the image into it, and use OpenCV to draw the facial keypoints on the output image and display the processed frame.
...
For video applications, the detector is invoked selectively, only when necessary. Specifically, it is used for the first frame or when the tracker fails to recognize the body pose in the preceding frame. In all other frames, the pipeline derives ROIs based on the pose landmarks detected in the previous.
...
Used to start the body key points detection.
...
```
(6) To close this function, press the "**Esc**" key in the image interface to exit the camera view.
(7) Then press "**Ctrl+C**" in the command line terminal interface. If closing fails, please try again.
...
The program's logic flowchart extracted from the program files is illustrated in the figure below.As depicted in the image above, the purpose of this game is to capture an image using the camera, preprocess it by converting its color space for easier identification, extract feature points corresponding to hand gestures from the converted image, and determine different gestures (based on angles) through logical analysis of key feature points. Finally, the trajectory of the recognized gesture is drawn on the display screen.
...
Draw the currently recognized hand shape and the keypoints.
...
Time synchronization callback function: takes depth information and RGB images as input, synchronizes the two, and then enqueues the synchronized data.
...
Track human function: uses the model to draw the person's position based on recognized information and employs PID control to adjust movement according to the detected person.
...
The program file is saved in: [ros2_ws/src/example/example/body_control/include/body_and_rgb_control.py](../_static/source_code/body_control.zip)
...
<img class="common_img" src="../_static/media/chapter_7/section_3/image5.jpeg" style="width:4.19444in;height:3.14583in" alt="" />

<img class="common_img" src="../_static/media/chapter_7/section_3/image6.jpeg" style="width:4.19306in;height:3.14444in" alt="" />

Turn right: Instruct the robot car to turn right.

<img class="common_img" src="../_static/media/chapter_7/section_3/image7.jpeg" style="width:4.19306in;height:3.14444in" alt="" />

Park: Command the robot car to park.

<img class="common_img" src="../_static/media/chapter_7/section_3/image8.jpeg" style="width:4.19306in;height:3.14444in" alt="" />
...
<img class="common_img" src="../_static/media/chapter_7/section_3/image9.jpeg" style="width:4.19306in;height:3.14444in" alt="" />
...
<img class="common_img" src="../_static/media/chapter_7/section_3/image10.jpeg" alt="" />
...
(1) Before starting, ensure the map is laid out flat and free of wrinkles, with smooth roads and no obstacles. For specific map laying instructions, please refer to "[**7.4 Map and Props Setup**](#anchor_7_4)" in the same directory as this section for guidance. (In this lesson, we are only experiencing the road driving-line patrol function, so there is no need to place props such as traffic lights and signboards.)
...
(3) It is essential to adjust the color threshold beforehand and set the color threshold of the yellow line to avoid misidentification during subsequent recognition. For specific color threshold adjustment, please refer to the "[**6. ROS2+OpenCV Course**](6.ros+opencv_course.md)" for reference.
...
① Before starting, ensure the map is laid out flat and free of wrinkles, with smooth roads and no obstacles. For specific map laying instructions, please refer to "[7.4 Map and Props Setup](#anchor_7_4)" in the same directory as this section for guidance.
...
② The roadmap model discussed in this section is trained using YOLOv5. For further information on YOLOv5 and related content, please refer to "[**7. ROS+Machine Learning Course\7.2 Machine Learning Application**](#anchor_7_2)".
...
Adjust the color threshold to better suit the lighting conditions of the actual scene. For precise instructions on color threshold adjustment, please consult "[**6. ROS+OpenCV Lesson**](6.ros+opencv_course.md)" for detailed guidance.

(2) The robot's turning radius appears to be either too large or too small.

①  Ensure correct adjustment of the robot arm deviation. For detailed instructions on robot arm deviation adjustment.
...
You can adjust the parking processing function or modify the starting position of the parking operation. For detailed instructions, please consult "[**7.5.5 Autonomous Parking->Parameter Adjustment**](#anchor_7_5_5_5)" for reference and learning.

(4) Inaccurate traffic sign recognition.

Adjust the target detection confidence. For detailed instructions, please refer to "[**7.5.5 Autonomous Parking->Operation Steps**](#anchor_7_5_5_3) " for comprehensive learning.