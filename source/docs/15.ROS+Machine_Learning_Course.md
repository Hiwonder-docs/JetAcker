...
For seamless usage of MediaPipe, we suggest gaining a basic understanding of OpenCV. To delve into OpenCV, you can find detailed information in "**[14. ROS+OpenCV Lesson]()**".
...
Before starting, ensure the map is laid out flat and free of wrinkles, with smooth roads and no obstacles. For specific map laying instructions, please refer to "**[0. Autonomous Driving Debugging Lesson]()**" in the same directory as this section for guidance. (In this lesson, we are only experiencing the road driving-line patrol function, so there is no need to place props such as traffic lights and signboards.)
...
It is essential to adjust the color threshold beforehand and set the color threshold of the yellow line to avoid misidentification during subsequent recognition. For specific color threshold adjustment, please refer to the "**[7. ROS+OpenCV Courses]()**" for reference.
...
Before starting, ensure the map is laid out flat and free of wrinkles, with smooth roads and no obstacles. For specific map laying instructions, please refer to "**[0. Autonomous Driving Debugging]()**" in the same directory as this section for guidance.
...
1)  Before starting, ensure the map is laid out flat and free of wrinkles, with smooth roads and no obstacles. For specific map laying instructions, please refer to "**[0. Autonomous Driving Debugging Lesson]()**" in the same directory as this section for guidance.
...
2)  The roadmap model discussed in this section is trained using YOLOv5. For further information on YOLOv5 and related content, please refer to "**[15.1.1 MediaPipe Introduction]()**”.
...
1. The robot exhibits inconsistent performance during line patrolling, often veering off course.
   ...
   Adjust the color threshold to better suit the lighting conditions of the actual scene. For precise instructions on color threshold adjustment, please consult "**[7 ROS+OpenCV Lesson]()**" for detailed guidance.
...
4. Inaccurate traffic sign recognition.
   ...
   Adjust the target detection confidence. For detailed instructions, please refer to "**[15.3.2 Operation Steps]()**" for comprehensive learning.
...
6. When adjusting the '**conf_thresh**' value does not yield satisfactory results, it may be necessary to retrain the YOLOv5 model. For specific training methods, you can refer to “**[15. ROS + Machine Learning\ 15.2.6 Machine Learning Applications]()”** for training the model. Once trained, load the model path into the launch file and restart the game.
